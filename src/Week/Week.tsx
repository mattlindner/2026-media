...
const Week = ({ week, groupedMedia, setSelectedMedia }: WeekProps) => {
	...
	const { media, movieScore, musicScore } = useMemo(() => {
		const weekMedia = week.flatMap((day) => groupedMedia[day.toISOString().split("T")[0]] || [])
	...
	return (
		<div className="week">
			<div className="date-range">{`${mdFormatter.format(week[0])} – ${mdFormatter.format(week[week.length - 1])}`}</div>
			...
			<div className="media">
				{media.map((m) => {
					const { titleLabel, title, subtitleLabel, subtitle } = getMediaLabels(m)
					return (
						<>
							<hr />
							<img
