...
function getWeeksInYear(year: number): Date[][] {
	...
	const yearEnd = year === today.getFullYear() ? currentWeekEnd : new Date(year, 11, 31)
...
const App = () => {
	...
	const groupedMedia = useMemo(() => Object.groupBy(media, (item) => item.date), [])

	return (
		<div>
			{weeks.map((week) => (
				<Week key={week[0].toISOString()} week={week} groupedMedia={groupedMedia} setSelectedMedia={setSelectedMedia} />
			))}
